...
一个典型的搜索引擎系统通常从文档采集开始[1]。在 Web 搜索引擎中，这一步由网络爬虫完成；在本系统中，对应的是解析 Cranfield 数据集的 XML 文件，并从结构化字段中提取标题、作者、出处和正文内容。文档进入系统后，需要经过预处理转换为适合索引的词项序列，常见步骤包括分词、大小写归一化、去停用词和词干提取。预处理策略会直接影响检索效果：过度激进的词干提取可能合并语义不同的词，过于保守又会降低召回率。
...
课堂上学习 TF-IDF、余弦相似度、Zipf 定律等概念时，公式和定理比较抽象。通过本次实践，我们实现了这些算法，并在真实数据集上观察其效果：词频分布近似符合 Zipf 定律，IDF 能降低高频通用词的权重，余弦相似度能将更相关的文档排在前面。经过这次实现，我们对信息检索的基础理论有了更具体的理解，也对密集检索、RAG 等后续技术有了更清晰的参照。
